...
import { FolderIcon, FolderOpenIcon, XIcon, StarIcon, CheckCircleIcon } from './components/Icons';
...
const App: React.FC = () => {
  ...
  const getFolderColorClasses = (color?: string) => {
    switch (color) {
      ...
      case 'indigo':
      default:
        return { bg: 'bg-indigo-50 dark:bg-primary/15', text: 'text-indigo-700 dark:text-blue-300', border: 'border-indigo-200 dark:border-primary/40', dot: 'bg-indigo-500', active: 'bg-indigo-650 text-white' };
    }
  };
  ...
  const providerLabels: Record<'gemini' | 'openai' | 'anthropic', string> = {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900/60 dark:bg-slate-900 text-gray-900 dark:text-white dark:text-slate-100 font-sans flex flex-col">
      {/* Header */}
      <header className="bg-white dark:bg-slate-800 dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-3">
          <button
            className="flex items-center gap-2 cursor-pointer rounded-lg px-1 -ml-1 py-1 min-h-[44px]"
            onClick={() => setViewState('dashboard')}
            aria-label="Go to Dashboard"
          >
            <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center text-white font-bold flex-shrink-0">M</div>
            <h1 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">MatchEval</h1>
          </button>
          <div className="flex items-center gap-2 sm:gap-3 overflow-x-auto scrollbar-hide">
             {/* Provider status: visible at a glance, before the user tries to run anything */}
             <div
               className={`hidden sm:flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg font-bold select-none uppercase tracking-wide border ${
               ...
               title={hasProviderKey[activeProvider] ? `${providerLabels[activeProvider]} key is set` : `${providerLabels[activeProvider]} key is missing`}
             >
               <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${hasProviderKey[activeProvider] ? 'bg-emerald-500' : 'bg-amber-500 animate-pulse'}`} />
               {activeProvider}
             </div>
             <button
               onClick={() => setViewState('settings')}
               className={`text-sm font-semibold px-3 py-2 rounded-lg transition-colors cursor-pointer min-h-[40px] whitespace-nowrap ${viewState === 'settings' ? 'bg-primary/10 text-primary dark:bg-primary/20 dark:text-blue-300' : 'text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-800'}`}
             >
               System Settings
             </button>
             {viewState === 'editor' && (
               <>
                 <div className="flex bg-gray-100 dark:bg-slate-700 dark:bg-slate-800 p-1 rounded-lg border border-gray-200 dark:border-slate-700 gap-1 select-none">
                   <button
                     type="button"
                     onClick={() => handleExportConversation('json')}
                     title="Download JSON (Raw Conversation Data & Evaluation)"
                     className="px-2.5 py-1.5 text-xs font-bold text-gray-750 dark:text-slate-200 hover:bg-white dark:hover:bg-slate-700 rounded transition-colors cursor-pointer"
                   >
                     JSON
                   </button>
                   <button
        ...
        {viewState === 'dashboard' && (
          <div 
            className={`space-y-6 animate-in fade-in duration-500 rounded-2xl transition-all ${
              isDragging ? 'bg-indigo-50 dark:bg-primary/15/50 p-6 ring-4 ring-indigo-500/10 shadow-inner' : ''
            }`}
          ...
          >
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-slate-800 p-5 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-2xs">
              <div>
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Dashboard</h2>
                <p className="text-gray-500 dark:text-slate-400 text-xs mt-0.5">Organize conversations in folders, filter categories, and evaluate AI performance.</p>
              </div>
              <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto">
                 {/* Upload Target Folder Selector */}
                 <div className="flex items-center gap-1.5 text-xs font-semibold bg-gray-50 dark:bg-slate-800 border border-gray-250 dark:border-slate-600 rounded-lg px-2.5 py-1.5 shadow-2xs">
                   <span className="text-gray-500 dark:text-slate-400 font-bold hidden sm:inline">Upload into:</span>
                   <select
                     ...
                     className="bg-transparent border-none text-xs font-bold text-indigo-700 dark:text-blue-300 focus:ring-0 focus:outline-none cursor-pointer pr-1"
                   >
            ...
            <div className="bg-white dark:bg-slate-800 p-3.5 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-2xs space-y-2.5 select-none">
              <div className="flex items-center justify-between px-1">
                <span className="text-xs font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                ...
                <button
                  ...
                  className="text-xs font-bold text-indigo-650 hover:text-indigo-800 flex items-center gap-1 hover:underline cursor-pointer bg-indigo-50 dark:bg-primary/15 hover:bg-indigo-100/80 px-2.5 py-1 rounded-lg transition-colors"
                >
              ...
              <div className="flex gap-2 flex-wrap items-center">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-all duration-150 flex items-center gap-2 cursor-pointer ${
                    selectedFolderId === 'ALL'
                      ? 'bg-indigo-50 dark:bg-primary/30 text-indigo-950 dark:text-white border-indigo-400 dark:border-primary/60 shadow-xs ring-2 ring-indigo-200/70 dark:ring-primary/40'
                      : 'bg-slate-50 dark:bg-slate-900/60 text-gray-700 dark:text-slate-300 border-gray-200 dark:border-slate-700 hover:bg-gray-100'
                  }`}
                >
                  <span className="flex items-center gap-1.5"><FolderIcon size={13} /> All Conversations</span>
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-black ${selectedFolderId === 'ALL' ? 'bg-indigo-600 text-white' : 'bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-slate-300'}`}>
                ...
                >
                  <span className="flex items-center gap-1.5"><FolderOpenIcon size={13} /> Unassigned</span>
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-black ${selectedFolderId === 'UNASSIGNED' ? 'bg-indigo-600 text-white' : 'bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-slate-300'}`}>
                  ...
                  return (
                    <div key={folder.id} className="relative group">
                      ...
                      >
                        ...
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-extrabold ${isSelected ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-slate-800/90 text-gray-700 dark:text-slate-300 border border-gray-200 dark:border-slate-700'}`}>
                          {count}
                        </span>
                      </button>
                      <button
                        ...
                        title="Delete folder"
                        className={`absolute right-1.5 top-1/2 -translate-y-1/2 p-0.5 opacity-0 group-hover:opacity-100 transition-opacity rounded ${isSelected ? 'text-indigo-500 hover:text-red-600' : 'text-gray-400 dark:text-slate-500 hover:text-red-500'}`}
                      >
            ...
            {isDragging && conversations.length > 0 && (
              <div className="py-12 text-center bg-indigo-50 dark:bg-primary/15/80 rounded-2xl border-2 border-dashed border-indigo-500 text-indigo-800 backdrop-blur-xs flex flex-col items-center justify-center space-y-2 animate-in zoom-in duration-200">
                <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="animate-bounce text-indigo-600 dark:text-blue-400"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="17 8 12 3 7 8"></polyline><line x1="12" y1="3" x2="12" y2="15"></line></svg>
                <p className="font-extrabold text-lg">Drop your JSON, JSONL, or TXT file to import conversation</p>
                <p className="text-xs font-semibold text-indigo-600 dark:text-blue-400">Supports MatchEval exports, JSONL datasets, raw message arrays, or formatted transcripts</p>
              </div>
            )}

            {conversations.length > 0 && (
              <div className="mb-6 bg-white dark:bg-slate-800 p-4 rounded-xl border border-gray-200 dark:border-slate-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-xs select-none animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="flex flex-wrap items-center gap-3">
                  <div className="flex items-center h-5">
                    <input 
                      ...
                      className="h-4 w-4 text-indigo-600 dark:text-blue-400 rounded border-gray-300 dark:border-slate-600 focus:ring-indigo-500 cursor-pointer"
                      disabled={bulkEvalProgress?.running}
                    />
                  </div>
                  <label htmlFor="selectAll" className="text-sm font-bold text-gray-800 dark:text-slate-200 cursor-pointer">
                  ...
                  {selectedIds.length > 0 && selectedIds.length < conversations.length && (
                    <span className="text-xs bg-indigo-50 dark:bg-primary/15 border border-indigo-100 dark:border-primary/30 text-indigo-700 dark:text-blue-300 px-2.5 py-1 rounded-full font-bold">
                ...
                {bulkEvalProgress && bulkEvalProgress.running ? (
                  <div
                    role="status"
                    aria-live="polite"
                    className="flex flex-col gap-2 bg-primary/5 dark:bg-primary/10 border border-primary/20 dark:border-primary/30 rounded-xl py-3 px-4 w-full sm:w-auto flex-1 max-w-xl sm:ml-4 shadow-2xs"
                  >
                    <div className="flex items-center justify-between gap-3 flex-wrap">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-xs font-extrabold text-primary dark:text-blue-200 tracking-tight whitespace-nowrap">
                          Analyzing conversation {bulkEvalProgress.current} of {bulkEvalProgress.total}
                        </span>
                        <span className="text-[10px] font-mono tabular-nums font-bold bg-white dark:bg-slate-800 text-primary dark:text-blue-300 border border-primary/20 dark:border-primary/40 px-1.5 py-0.5 rounded-full">
                          {Math.round((bulkEvalProgress.current / bulkEvalProgress.total) * 100)}%
                        </span>
                      </div>
                      <button
                        onClick={() => { cancelBulkRef.current = true; }}
                        className="text-[11px] text-red-600 dark:text-red-400 font-black hover:underline cursor-pointer uppercase tracking-wider px-2 py-1.5 rounded-md hover:bg-red-50 dark:hover:bg-red-950/40 min-h-[32px]"
                      >
                        Cancel remaining
                      </button>
                    </div>
                    ...
                    <div className="h-2.5 bg-primary/10 dark:bg-primary/20 rounded-full w-full overflow-hidden">
                      <div
                    ...
                    <div className="flex items-center justify-between gap-3 flex-wrap text-[11px] text-gray-500 dark:text-slate-400">
                      <span>Running up to 3 evaluations in parallel. You can keep browsing — this won't lock the page.</span>
                      {bulkEvalProgress.errorCount > 0 && (
                        <span className="inline-flex items-center gap-1 font-bold text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900 px-2 py-0.5 rounded-full">
                          {bulkEvalProgress.errorCount} failed so far
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div
                    role="status"
                    aria-live="polite"
                    className={`flex items-center gap-2.5 rounded-xl py-2.5 px-4 w-full sm:w-auto flex-1 max-w-xl sm:ml-4 border animate-in fade-in duration-200 ${
                      bulkEvalProgress.errorCount > 0
                  ...
                  >
                    {bulkEvalProgress.errorCount > 0 ? (
                      <span className="text-amber-600 dark:text-amber-400 flex-shrink-0"><CheckCircleIcon size={16} /></span>
                    ) : (
                      <span className="text-emerald-600 dark:text-emerald-400 flex-shrink-0"><CheckCircleIcon size={16} /></span>
                    )}
                    <span className={`text-xs font-bold ${bulkEvalProgress.errorCount > 0 ? 'text-amber-800 dark:text-amber-300' : 'text-emerald-800 dark:text-emerald-300'}`}>
                      Bulk evaluation finished: {bulkEvalProgress.total - bulkEvalProgress.errorCount} succeeded
                      {bulkEvalProgress.errorCount > 0 ? `, ${bulkEvalProgress.errorCount} failed (check conversations for details)` : ''}.
                    </span>
                  </div>
                ) : (
                  selectedIds.length > 0 && (
                    <div className="flex flex-wrap gap-2 w-full sm:w-auto items-center justify-end">
                      ...
                      <select
                        ...
                        className="bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-300 text-xs font-bold rounded-lg px-2.5 py-1.5 cursor-pointer focus:ring-2 focus:ring-indigo-500 shadow-2xs hover:bg-gray-50"
                      >
                      ...
                      <span className="text-[11px] text-gray-400 dark:text-slate-500 self-center mr-1 font-extrabold uppercase tracking-wider hidden sm:block">Export:</span>
            ...
            {conversations.length > 0 && (
              <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between select-none bg-slate-50 dark:bg-slate-900/60 p-2.5 rounded-xl border border-gray-200 dark:border-slate-700 shadow-2xs">
                {/* Category Filters */}
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[11px] font-extrabold text-gray-400 dark:text-slate-500 uppercase tracking-widest ml-1 flex items-center gap-1">
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    {(['All', 'Normal', 'Edge Case', 'Multilingual', 'Sensitive', 'Uncategorized'] as const).map(cat => {
                      ...
                      return (
                        ...
                        >
                          <span>{cat}</span>
                          <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-bold ${categoryFilter === cat ? 'bg-indigo-700 text-indigo-50' : 'bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-slate-400'}`}>{count}</span>
                        </button>
                      );
                    })}
                  </div>
                ...
                <div className="flex bg-white dark:bg-slate-800 p-1 rounded-xl border border-gray-250 dark:border-slate-600 shadow-2xs gap-1 self-end sm:self-center">
                  <button
                    onClick={() => handleViewModeChange('grid')}
                    className={`px-2.5 py-1 rounded-lg text-xs font-extrabold flex items-center gap-1.5 transition-all cursor-pointer ${
                      viewMode === 'grid' ? 'bg-indigo-650 text-white shadow-2xs' : 'text-gray-500 dark:text-slate-400 hover:text-gray-800 hover:bg-gray-100'
                    }`}
                  ...
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-lg text-xs font-extrabold flex items-center gap-1.5 transition-all cursor-pointer ${
                      viewMode === 'list' ? 'bg-indigo-650 text-white shadow-2xs' : 'text-gray-500 dark:text-slate-400 hover:text-gray-800 hover:bg-gray-100'
                    }`}
                    title="List View"
                  >
            ...
            {(() => {
              ...
              if (conversations.length === 0) {
                return (
                  <div className={`py-20 text-center bg-white dark:bg-slate-800 rounded-2xl border-2 border-dashed ${isDragging ? 'border-indigo-600 bg-indigo-50 dark:bg-primary/15/20 text-indigo-700 dark:text-blue-300' : 'border-gray-200 dark:border-slate-700'} transition-all`}>
                      <div className="flex flex-col items-center justify-center space-y-3">
                        <div className={`p-4 rounded-full bg-indigo-50 dark:bg-primary/15 text-indigo-600 dark:text-blue-400 ${isDragging ? 'scale-110 bg-indigo-100 text-indigo-800 animate-pulse' : ''} transition-all`}>
                        ...
                        <p className="text-gray-500 dark:text-slate-400 text-sm max-w-sm px-4 leading-relaxed">
              ...
              if (filteredList.length === 0) {
                return (
                  <div className="py-16 text-center bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700">
                    <div className="flex flex-col items-center justify-center space-y-2 animate-in zoom-in-95 duration-200">
                      <div className="p-3.5 rounded-full bg-slate-50 dark:bg-slate-900/60 text-slate-400">
                      ...
                      <p className="text-gray-950 dark:text-white font-bold">No conversations in this folder or category</p>
                      <p className="text-gray-500 dark:text-slate-400 text-xs">Try switching folders or clearing category filters.</p>
                    ...
                    {filteredList.map(conv => {
                      ...
                      return (
                       <div 
                         ...
                         className={`group bg-white dark:bg-slate-800 rounded-xl border p-5 hover:shadow-lg transition-all cursor-pointer relative flex flex-col justify-between ${selectedIds.includes(conv.id) ? 'border-indigo-500 ring-2 ring-indigo-500/10 bg-indigo-50 dark:bg-primary/15/5' : 'border-gray-200 dark:border-slate-700'}`}
                       >
                         <div>
                           <div className="flex justify-between items-start mb-3 gap-2">
                              <div className="flex items-center gap-2.5 flex-1 min-w-0" onClick={(e) => e.stopPropagation()}>
                                <input 
                                  ...
                                  className="h-4 w-4 text-indigo-600 dark:text-blue-400 rounded border-gray-300 dark:border-slate-600 focus:ring-indigo-500 cursor-pointer flex-shrink-0"
                                />
                                <h3 className="font-bold text-base text-gray-900 dark:text-white line-clamp-1 truncate">{conv.title}</h3>
                              </div>
                              <button onClick={(e) => deleteConversation(conv.id, e)} className="text-gray-400 dark:text-slate-500 hover:text-red-500 p-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 cursor-pointer">
                           ...
                           <div className="flex flex-wrap gap-1.5 items-center mb-4" onClick={(e) => e.stopPropagation()}>
                             <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border select-none ${
                               ...
                               conv.category === 'Sensitive'   ? 'bg-rose-50 text-rose-700 border-rose-200' :
                                                                 'bg-slate-50 dark:bg-slate-900/60 text-slate-600 border-slate-200'
                             }`}>
                               <div className="flex items-center gap-1 py-0.5">
                                 ...
                                 >
                                   <option value="Uncategorized" className="text-gray-950 dark:text-white font-bold bg-white dark:bg-slate-800">Uncategorized</option>
                                   <option value="Normal" className="text-gray-950 dark:text-white font-bold bg-white dark:bg-slate-800">Normal</option>
                                   <option value="Edge Case" className="text-gray-950 dark:text-white font-bold bg-white dark:bg-slate-800">Edge Case</option>
                                   <option value="Multilingual" className="text-gray-950 dark:text-white font-bold bg-white dark:bg-slate-800">Multilingual</option>
                                   <option value="Sensitive" className="text-gray-950 dark:text-white font-bold bg-white dark:bg-slate-800">Sensitive</option>
                                 </select>
                               </div>
                             </span>

                             <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border select-none ${
                               folder ? `${folderColorInfo.bg} ${folderColorInfo.text} ${folderColorInfo.border}` : 'bg-gray-50 dark:bg-slate-800 text-gray-500 dark:text-slate-400 border-gray-200 dark:border-slate-700'
                             }`}>
                               <FolderIcon size={11} className="mr-1" />
                               <select
                                 value={conv.folderId || ''}
                                 onChange={(e) => {
                                   if (e.target.value === '__NEW__') {
                               ...
                               >
                                 ...
                                 {folders.map(f => (
                                   <option key={f.id} value={f.id} className="text-gray-900 dark:text-white bg-white dark:bg-slate-800">{f.name}</option>
                                 ))}
                                 <option value="__NEW__" className="text-indigo-600 dark:text-blue-400 font-bold bg-white dark:bg-slate-800">+ New Folder...</option>
                               </select>
                             </span>
                           </div>
                         </div>
                        
                         <div className="flex justify-between items-end pt-3 border-t border-gray-100 dark:border-slate-700">
                            ...
                            {conv.evaluation ? (
                              <div className="text-right">
                                <div className="text-xl font-black text-indigo-600 dark:text-blue-400">{conv.evaluation.overallScore}<span className="text-xs text-gray-400 dark:text-slate-500 font-normal">/10</span></div>
                                <div className="text-[10px] text-gray-400 dark:text-slate-500 uppercase font-bold tracking-wide">Score</div>
                              </div>
                            ) : (
                              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-300">
                                Draft
                              </span>
                            )}
                         </div>
              ...
              return (
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden shadow-2xs">
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                      <thead className="bg-slate-50 dark:bg-slate-900/60 text-gray-500 dark:text-slate-400 text-[11px] uppercase tracking-wider font-extrabold border-b border-gray-200 dark:border-slate-700 select-none">
                        <tr>
                        ...
                        {filteredList.map(conv => {
                          ...
                          return (
                            <tr
                              key={conv.id}
                              onClick={() => handleSelectConversation(conv)}
                              className={`hover:bg-slate-50/80 transition-colors cursor-pointer ${isSelected ? 'bg-indigo-50 dark:bg-primary/15/30' : ''}`}
                            >
                              ...
                              <td className="py-3.5 px-4 max-w-xs md:max-w-md">
                                <div className="font-bold text-gray-900 dark:text-white truncate text-sm">{conv.title}</div>
                                <div className="text-xs text-gray-400 dark:text-slate-500 truncate mt-0.5">{firstMsg}</div>
                              </td>
                              ...
                              <td className="py-3.5 px-4 whitespace-nowrap">
                                {conv.evaluation ? (
                                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-black bg-indigo-50 dark:bg-primary/15 text-indigo-700 dark:text-blue-300 border border-indigo-100 dark:border-primary/30">
                                    <StarIcon size={11} /> {conv.evaluation.overallScore} / 10
                                  </span>
                                ) : (
                                  <span className="text-xs font-semibold text-gray-400 dark:text-slate-500">Unanalyzed</span>
                                )}
                              </td>
                              <td className="py-3.5 px-4 whitespace-nowrap text-xs text-gray-500 dark:text-slate-400">
                                <div>{conv.messages.length} turns</div>
                                <div className="text-[10px] text-gray-400 dark:text-slate-500">{new Date(conv.createdAt).toLocaleDateString()}</div>
                              </td>
                              <td className="py-3.5 px-4 whitespace-nowrap text-right" onClick={(e) => e.stopPropagation()}>
                                <div className="flex items-center justify-end gap-2">
                                  <button
                                    onClick={() => handleSelectConversation(conv)}
                                    className="text-xs font-bold text-indigo-600 dark:text-blue-400 hover:text-indigo-800 bg-indigo-50 dark:bg-primary/15 hover:bg-indigo-100 px-2.5 py-1 rounded-lg transition-colors cursor-pointer"
                                  >
                                    View / Edit
                                  </button>
                                  <button
                                    onClick={(e) => deleteConversation(conv.id, e)}
                                    className="text-gray-400 dark:text-slate-500 hover:text-red-500 p-1 transition-colors cursor-pointer"
                                  >
                                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18"></path><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>
                                  </button>
                                </div>
            ...
            <Modal isOpen={showNewFolderModal} onClose={() => setShowNewFolderModal(false)} labelledBy="new-folder-title" className="select-none">
              <div className="flex justify-between items-center">
                <h3 id="new-folder-title" className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                  <span className="p-2 bg-primary/10 text-primary dark:bg-primary/20 dark:text-blue-300 rounded-xl"><FolderIcon size={16} /></span> Create New Folder
                </h3>
                <button
                  onClick={() => setShowNewFolderModal(false)}
                  aria-label="Close dialog"
                  className="text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:text-slate-400 dark:hover:text-slate-200 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 cursor-pointer min-w-[36px] min-h-[36px] flex items-center justify-center"
                >
                  <XIcon size={14} />
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label htmlFor="new-folder-name" className="block text-xs font-extrabold text-gray-600 dark:text-slate-300 dark:text-slate-400 uppercase tracking-wider mb-1.5">
                    Folder Name
                  </label>
                  <input
                    id="new-folder-name"
                    ...
                    className="w-full px-3.5 py-2.5 border border-gray-300 dark:border-slate-600 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500 rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-primary"
                    onKeyDown={(e) => {
                ...
                <div>
                  <span className="block text-xs font-extrabold text-gray-600 dark:text-slate-300 dark:text-slate-400 uppercase tracking-wider mb-2">
                    Folder Tag Color
                  </span>
                  <div className="flex gap-2.5" role="radiogroup" aria-label="Folder tag color">
                    {(['indigo', 'emerald', 'amber', 'rose', 'sky', 'purple', 'slate'] as const).map(color => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          role="radio"
                          aria-checked={newFolderColor === color}
                          aria-label={`${color} color tag`}
                          onClick={() => setNewFolderColor(color)}
                          className={`w-9 h-9 sm:w-8 sm:h-8 rounded-full ${info.dot} flex items-center justify-center transition-transform duration-150 cursor-pointer ${
                            newFolderColor === color ? 'ring-2 ring-offset-2 dark:ring-offset-slate-800 ring-primary scale-110' : 'hover:scale-105'
                          }`}
                        >
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <Button
                  ...
                  className="bg-primary hover:bg-primary/90 text-white font-bold px-4 cursor-pointer"
                >
                  Create Folder
                </Button>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start pb-12 animate-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="lg:col-span-7 flex flex-col lg:sticky lg:top-24 lg:h-[calc(100vh-8rem)]">
                <div className="mb-4 flex flex-col gap-1.5 flex-shrink-0 animate-in fade-in duration-350">
                   <div className="flex justify-between items-center">
                      <h2 className="text-xl font-bold text-gray-800 dark:text-slate-200 truncate pr-4">{currentConversation.title}</h2>
                ...
                <div className="flex-1 min-h-0 overflow-hidden rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800">
                  <ChatInterface 
              ...
              <div className="lg:col-span-5 space-y-6">
                    ...
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="font-semibold text-gray-900 dark:text-white">Evaluation Config</h3>
                            <Button 
                              variant="ghost" 
                              size="sm" 
                              className="text-xs text-indigo-600 dark:text-blue-400"
                        ...
                        {showContextInputs && (
                          <div className="space-y-4 mb-6 animate-in fade-in slide-in-from-top-2">
                             <div>
                               <label className="block text-xs font-semibold text-gray-500 dark:text-slate-400 mb-1">Domain Context / Knowledge Base</label>
                               <textarea
                                 className="w-full text-sm p-3 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-indigo-500"
                                 rows={3}
                             ...
                             <div>
                               <label className="block text-xs font-semibold text-gray-500 dark:text-slate-400 mb-1">Ground Truth / Reference Answer</label>
                               <textarea
                                 className="w-full text-sm p-3 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-indigo-500"
                                 rows={3}
                        ...
                        <div className="mb-4">
                          ...
                          <div className="grid grid-cols-3 gap-1 bg-slate-50 dark:bg-slate-900/60 p-1 rounded-lg border border-gray-200 dark:border-slate-700">
                            <button
                              type="button"
                          ...
                          {!hasProviderKey[activeProvider] && (
                            <div className="mt-2 text-xs bg-amber-50 text-amber-800 border border-amber-200 p-2.5 rounded-lg flex items-start gap-2.5 leading-normal animate-in fade-in slide-in-from-top-1">
                              ...
                              <div>
                                ...
                                <button 
                                  onClick={() => setViewState('settings')}
                                  className="text-indigo-600 dark:text-blue-400 hover:underline font-bold ml-1 inline-block"
                                >
                        ...
                        {activeProvider === 'anthropic' ? (
                          <div className="mb-4 rounded-lg border border-indigo-100 dark:border-primary/30 bg-indigo-50 dark:bg-primary/15 p-3 text-xs leading-relaxed text-indigo-800">
                            Claude sampling is managed automatically. MatchEval omits the deprecated temperature parameter for Anthropic evaluations.
                          </div>
                        ) : (
                        <div className="mb-4 bg-slate-50 dark:bg-slate-900/60 p-3 rounded-lg border border-gray-200 dark:border-slate-700">
                          <div className="flex justify-between items-center mb-1.5">
                            <label className="text-xs font-bold text-gray-700 dark:text-slate-300 uppercase tracking-wider">
                              Judge Temperature: <span className="font-extrabold text-indigo-700 dark:text-blue-300">{appTemperature.toFixed(1)}</span>
                            </label>
                            <span className="text-[10px] bg-indigo-50 dark:bg-primary/15 text-indigo-700 dark:text-blue-300 border border-indigo-100 dark:border-primary/30 font-bold px-1.5 py-0.5 rounded font-sans uppercase">
                              {appTemperature === 0 ? 'Deterministic' : appTemperature >= 0.7 ? 'Creative' : 'Balanced'}
                            </span>
                          </div>
                          
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] text-gray-400 dark:text-slate-500 font-mono">0.0</span>
                            <input
                              type="range"
                              min="0"
                              max="1"
                            ...
                            />
                            <span className="text-[10px] text-gray-400 dark:text-slate-500 font-mono">1.0</span>
                          </div>
                          <p className="mt-1.5 text-[10px] text-gray-500 dark:text-slate-400 leading-relaxed font-sans">
                            {appTemperature === 0 
                        ...
                        <div className="flex items-center gap-3 mb-4 p-3 bg-gray-50 dark:bg-slate-800 rounded-lg border border-gray-100 dark:border-slate-700">
                           <input 
                             type="checkbox" 
                             id="factCheck" 
                             checked={enableFactCheck} 
                             onChange={(e) => setEnableFactCheck(e.target.checked)}
                             className="h-4 w-4 text-indigo-600 dark:text-blue-400 rounded focus:ring-indigo-500 border-gray-300 dark:border-slate-600"
                           />
                           <label htmlFor="factCheck" className="text-sm text-gray-700 dark:text-slate-300 cursor-pointer select-none">
                    ...
                    {currentConversation.evaluation && (
                      <>
                        {/* Overall Score Card */}
                        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 relative overflow-hidden">
                           <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-50 dark:bg-primary/15 rounded-full blur-3xl -mr-16 -mt-16"></div>
                           <div className="relative z-10 flex items-center justify-between">
                             <div>
                               <div className="text-4xl font-bold text-gray-900 dark:text-white mb-1">{currentConversation.evaluation.overallScore}</div>
                               <div className="text-sm font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wide">Overall Score</div>
                             </div>
                           ...
                           <div className="mt-4 pt-4 border-t border-gray-100 dark:border-slate-700">
                             <div className="flex justify-between items-center mb-2">
                                ...
                                <div className="flex bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 p-0.5 rounded gap-0.5 text-[10px] select-none">
                                  <button
                                    type="button"
                                    onClick={() => handleExportConversation('json')}
                        ...
                        {currentConversation.evaluation.factCheckReport && (
                          <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-orange-100 relative">
                             ...
                             {currentConversation.evaluation.factCheckSources && currentConversation.evaluation.factCheckSources.length > 0 && (
                               <div className="text-xs">
                                 <div className="font-semibold text-gray-500 dark:text-slate-400 mb-1">Sources:</div>
                        ...
                        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700">
                            <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-4">Metric Breakdown</h4>
                                ...
                                {currentConversation.evaluation.metrics.map(metric => (
                                    <div key={metric.name}>
                                        <div className="flex justify-between items-center mb-1">
                                            <span className="text-sm font-medium text-gray-700 dark:text-slate-300">{metric.name}</span>
                                        ...
                                        <p className="text-xs text-gray-500 dark:text-slate-400">{metric.reasoning}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        {currentConversation.evaluation.suggestedImprovements && (
                            <div className="bg-gradient-to-br from-indigo-50 to-white p-6 rounded-xl shadow-sm border border-indigo-100 dark:border-primary/30">
                                <h4 className="flex items-center gap-2 text-sm font-bold text-indigo-900 dark:text-blue-200 mb-3">
      ...
      {/* Confirmation Modal — destructive actions require an explicit choice */}
      <Modal
        isOpen={!!(confirmModal && confirmModal.isOpen)}
      ...
      >
        {confirmModal && (
          <>
            <div id="confirm-modal-title" className="flex items-center gap-3 text-red-600 dark:text-red-400 font-bold text-lg">
              <div className="p-2.5 bg-red-100 dark:bg-red-950/50 text-red-600 dark:text-red-400 rounded-xl flex-shrink-0">
            ...
            <div className="flex justify-end gap-2.5 pt-2">
              <button
                ...
                className="px-4 py-2.5 rounded-xl text-xs font-bold text-gray-600 dark:text-slate-300 dark:text-slate-200 bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors cursor-pointer min-h-[40px]"
              >
                Cancel
              </button>
              <button
                onClick={() => confirmModal.onConfirm()}
                className="px-4 py-2.5 rounded-xl text-xs font-bold text-white bg-destructive hover:bg-red-700 shadow-sm transition-colors cursor-pointer min-h-[40px]"
              >
