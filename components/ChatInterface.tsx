...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden relative">
      <div 
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`group flex ${msg.role === Role.USER ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] lg:max-w-[75%] flex flex-col ${msg.role === Role.USER ? 'items-end' : 'items-start'}`}>
              ...
              {/* Role Label */}
              <span className="text-xs text-gray-400 dark:text-slate-500 mb-1 ml-1 uppercase font-semibold tracking-wider">
                {msg.role}
              </span>
              ...
              <div 
                className={`
                  ...
                  ${msg.role === Role.USER 
                    ...
                    : 'bg-gray-50 dark:bg-slate-800 text-gray-800 dark:text-slate-200 border-gray-100 dark:border-slate-700 rounded-tl-none'}
              ...
              >
                {editingId === msg.id ? (
                  <div className="min-w-[300px]">
                    <textarea
                      ...
                      className="w-full h-32 p-3 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-gray-900 dark:text-white bg-white dark:bg-slate-800 placeholder-gray-400 shadow-sm"
                      autoFocus
                    />
                ...
                {!readOnly && editingId !== msg.id && (
                  <div className="absolute -top-3 right-2 hidden group-hover:flex gap-1 bg-white dark:bg-slate-800 shadow-sm rounded-full border border-gray-100 dark:border-slate-700 p-1 transform translate-y-[-50%] z-10">
                    ...
                    <div className="w-px h-5 bg-gray-200 dark:bg-slate-700 mx-0.5"></div>
                    <button title="Add Comment" onClick={(e) => { e.stopPropagation(); setCommentInput({ id: msg.id, text: '' }); }} className="p-0.5 text-gray-500 dark:text-slate-400 hover:text-blue-600">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>
                    </button>
                    <button title="Edit Text" onClick={(e) => { e.stopPropagation(); handleEditStart(msg); }} className="p-0.5 text-gray-500 dark:text-slate-400 hover:text-blue-600">
              ...
              <div className="mt-2 w-full space-y-2">
                {(msg.comments || []).map(comment => (
                  <div key={comment.id} className="bg-yellow-50 border border-yellow-100 p-2 rounded-lg text-xs text-gray-700 dark:text-slate-300 relative group/comment">
                ...
                {commentInput && commentInput.id === msg.id && (
                  <div className="flex gap-2 items-center mt-2 animate-in fade-in slide-in-from-top-1">
                    <input 
                      type="text" 
                      className="flex-1 text-sm border border-gray-300 dark:border-slate-600 rounded px-2 py-1 focus:ring-1 focus:ring-blue-500 outline-none bg-white dark:bg-slate-800"
      ...
      {showScrollButton && (
        <button
          ...
          className="absolute bottom-6 right-6 p-3 bg-white dark:bg-slate-800 text-indigo-600 dark:text-blue-400 rounded-full shadow-lg border border-gray-100 dark:border-slate-700 hover:bg-gray-50 transition-all animate-in fade-in zoom-in duration-200 z-20"
